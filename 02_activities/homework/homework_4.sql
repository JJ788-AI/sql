...
SELECT 
product_name || ', ' ||coalesce(ifnull( product_size,' ') || ' (' || product_qty_type, 'unit' )|| ')'
FROM product
...
SELECT customer_id, market_date, customer_visit AS [customer_most_recent _visit]
FROM 
(
SELECT customer_id, market_date,
...
SELECT  customer_id, product_id, COUNT (*) as purchase_frequency 
FROM 
(
SELECT customer_id, product_id, market_date,
ROW_NUMBER( ) OVER (PARTITION BY product_id ORDER BY customer_id) AS product_purchase 
FROM customer_purchases
)X
GROUP BY 
customer_id,
...
SELECT  market_date, sales AS [best_day]
FROM
(
...
SELECT market_date, sales AS [worst_day]
FROM
(
SELECT market_date, sales,
ROW_NUMBER( ) OVER (ORDER BY sales) AS sales_rank
